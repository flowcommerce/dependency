...
trait SimpleScalaParser {
  ...
  /** Full path to the file from this project's repository that we parsed. Useful for the user to identify from where
    * the actual dependency came.
    */
  def path: String
  ...
  /** Parses into meaningful lines of data, stripping comments and removing blank lines
    */
  def parseIntoLines(contents: String): Seq[String] = {
    contents.split("\n").map(stripComments).map(_.trim).toSeq.filter(!_.isEmpty)
  }
  ...
  /** This method will substitute any variables for their values. For literals, we strip the quotes.
    */
  ...
  def parseLibraries(): Seq[Artifact] = {
    ...
    lines
      ...
      .distinct
      .sortBy { l => (l.groupId, l.artifactId, l.version) }
      .toList
  }
