 ...
 * Service version: 0.5.69
 * apibuilder 0.14.3 app.apibuilder.io/flow/common/0.5.69/anorm_2_6_parsers
 */
import anorm._
...
package io.flow.common.v0.anorm.parsers {
  ...
  object MerchantOfRecordEntity {
    ...
    def parser(
      ...
      SqlParser.str(prefixOpt.getOrElse("") + name) ~
      io.flow.common.v0.anorm.parsers.VatRegistration.parserWithPrefix(prefixOpt.getOrElse("") + vatPrefix) ~
      SqlParser.get[Seq[String]](prefixOpt.getOrElse("") + streets) ~
      SqlParser.str(prefixOpt.getOrElse("") + city) ~
      SqlParser.str(prefixOpt.getOrElse("") + province).? ~
      SqlParser.str(prefixOpt.getOrElse("") + postal).? ~
  ...
  object VatRegistration {
    ...
    def parserWithPrefix(prefix: String, sep: String = "_"): RowParser[io.flow.common.v0.models.VatRegistration] = parser(prefixOpt = Some(s"$prefix$sep"))

    def parser(
      number: String = "number",
      country: String = "country",
      prefixOpt: Option[String] = None
    ): RowParser[io.flow.common.v0.models.VatRegistration] = {
      SqlParser.str(prefixOpt.getOrElse("") + number) ~
      SqlParser.str(prefixOpt.getOrElse("") + country) map {
        case number ~ country => {
          io.flow.common.v0.models.VatRegistration(
            number = number,
            country = country
          )
        }
