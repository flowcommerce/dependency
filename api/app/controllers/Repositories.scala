...
class Repositories @javax.inject.Inject() (
  ...
  def getGithub(
    ...
    if (!existingProject.isEmpty && organizationId.isEmpty) {
      ...
      val actualLimit = if (offset == 0 && !name.isEmpty && !owner.isEmpty) { 1 }
      else { limit }

      github
        .repositories(request.user, offset, actualLimit) { r =>
          (name match {
            case None => true
          ...
          (org match {
            ...
            case Some(org) => {
              ...
              existingProject == Some(true) && !projectsDao
                .findByOrganizationKeyAndName(auth, org.id, r.name)
                .isEmpty ||
