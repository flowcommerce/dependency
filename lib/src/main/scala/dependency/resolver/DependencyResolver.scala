...
case class DependencyResolution(
  ...
  private[this] lazy val allResolvedLibraries: Set[String] =
    resolved.flatMap(_.flatMap { p => p.provides.map(_.identifier) }).toSet

  def isLibraryResolved(identifier: String): Boolean = allResolvedLibraries.contains(identifier)

  def allUnknownLibraries: Seq[LibraryReference] =
    resolved.flatMap(_.flatMap(_.unknownLibraries)).distinct.sortBy(_.identifier)
